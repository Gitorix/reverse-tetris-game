...
async function emitClearSound(ctx:AudioContext,count:number){await ctx.resume();const now=ctx.currentTime+.03,output=ctx.createDynamicsCompressor();output.threshold.value=-12;output.knee.value=8;output.ratio.value=6;output.connect(ctx.destination);const sets=[[36],[48,55],[55,62,67,74],[48,55,60,64,67,72,76,79,84]];const notes=sets[Math.min(4,count)-1];notes.forEach((note,i)=>{const osc=ctx.createOscillator(),gain=ctx.createGain(),start=now+i*(count===4?.055:.1);osc.type=count===1?'sine':count===4?'sawtooth':'square';osc.frequency.value=440*Math.pow(2,(note-69)/12);gain.gain.setValueAtTime(count===1?.16:count===4?.34:.25,start);gain.gain.exponentialRampToValueAtTime(.001,start+(count===4?.5:.32));osc.connect(gain).connect(output);osc.start(start);osc.stop(start+.52)});if(count>=3){const boom=ctx.createOscillator(),g=ctx.createGain();boom.type='sine';boom.frequency.setValueAtTime(count===4?170:115,now);boom.frequency.exponentialRampToValueAtTime(35,now+.38);g.gain.setValueAtTime(count===4?.42:.24,now);g.gain.exponentialRampToValueAtTime(.001,now+.58);boom.connect(g).connect(output);boom.start(now);boom.stop(now+.6)}if(count===4){const buffer=ctx.createBuffer(1,ctx.sampleRate*.35,ctx.sampleRate),data=buffer.getChannelData(0);for(let i=0;i<data.length;i++)data[i]=(Math.random()*2-1)*Math.pow(1-i/data.length,3);const noise=ctx.createBufferSource(),g=ctx.createGain();noise.buffer=buffer;g.gain.setValueAtTime(.2,now);g.gain.exponentialRampToValueAtTime(.001,now+.35);noise.connect(g).connect(output);noise.start(now)}}
...
export default function Home() {
  ...
  const testClearSound=()=>{const AudioCtx=window.AudioContext||(window as typeof window & {webkitAudioContext:typeof AudioContext}).webkitAudioContext;if(AudioCtx&&!audioRef.current)audioRef.current=new AudioCtx();setMusicOn(true);if(audioRef.current)void emitClearSound(audioRef.current,4)};

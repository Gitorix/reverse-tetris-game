...
const W = 10, H = 20, DEADLINE_ROWS = 2;
...
export default function Home() {
  ...
  const lock = useCallback((p: Piece, b: Cell[][], rev: boolean) => {
    ...
    const crossedDeadline=(nextRev?out.slice(H-DEADLINE_ROWS):out.slice(0,DEADLINE_ROWS)).some(row=>row.some(Boolean));
    if(crossedDeadline){setBoard(out);playGameOverSound();setScreen('over');return;}

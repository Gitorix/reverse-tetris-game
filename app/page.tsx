...
export default function Home() {
  ...
  const playClearSound = useCallback((count: number) => {
    ...
    void ctx.resume().then(()=>{const now=ctx.currentTime+.02;const sets=[[40],[52,59],[60,67,72,79],[48,60,67,72,76,79,84,88]];const notes=sets[Math.min(4,count)-1];notes.forEach((note,i)=>{const osc=ctx.createOscillator(),gain=ctx.createGain(),start=now+i*(count===4?.05:.085);osc.type=count===1?'sine':count===4?'square':'triangle';osc.frequency.setValueAtTime(440*Math.pow(2,(note-69)/12),start);gain.gain.setValueAtTime(count===1?.13:count===4?.24:.19,start);gain.gain.exponentialRampToValueAtTime(.001,start+(count===4?.42:.28));osc.connect(gain).connect(ctx.destination);osc.start(start);osc.stop(start+.45)});if(count>=3){const boom=ctx.createOscillator(),g=ctx.createGain();boom.type='sine';boom.frequency.setValueAtTime(count===4?150:100,now);boom.frequency.exponentialRampToValueAtTime(38,now+.32);g.gain.setValueAtTime(count===4?.32:.17,now);g.gain.exponentialRampToValueAtTime(.001,now+.5);boom.connect(g).connect(ctx.destination);boom.start(now);boom.stop(now+.52)}});
  }, [musicOn]);

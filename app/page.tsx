...
function MinoBadge({kind='t',color='#28d7ff'}:{kind?:'t'|'l';color?:string}) { const shape=kind==='t'?[[0,0],[1,0],[2,0],[1,1]]:[[0,0],[0,1],[0,2],[1,2]]; return <span className={`banner-mino ${kind}`} aria-hidden="true" style={{'--mino-color':color} as React.CSSProperties}>{shape.map(([x,y],i)=><i key={i} style={{left:x*8,top:y*8}} />)}</span>; }
...
export default function Home() {
  ...
  const playClearSound = useCallback((count: number) => {
    ...
    if (!AudioCtx) return; const ctx = new AudioCtx(), now = ctx.currentTime;
    const sets = [[43],[55,62],[60,67,72,76],[48,60,67,72,76,79,84,88]]; const notes=sets[Math.min(4,count)-1];
    notes.forEach((note,i)=>{const osc=ctx.createOscillator(),gain=ctx.createGain(),start=now+i*(count===4?.045:.075);osc.type=count===1?'sine':count===4?'square':'triangle';osc.frequency.setValueAtTime(440*Math.pow(2,(note-69)/12),start);if(count===4)osc.frequency.exponentialRampToValueAtTime(440*Math.pow(2,(note-67)/12),start+.18);gain.gain.setValueAtTime(count===1?.065:.1,start);gain.gain.exponentialRampToValueAtTime(.001,start+(count===4?.38:.22));osc.connect(gain).connect(ctx.destination);osc.start(start);osc.stop(start+.4)});
    if(count===4){const bass=ctx.createOscillator(),gain=ctx.createGain();bass.type='sine';bass.frequency.setValueAtTime(120,now);bass.frequency.exponentialRampToValueAtTime(42,now+.28);gain.gain.setValueAtTime(.22,now);gain.gain.exponentialRampToValueAtTime(.001,now+.45);bass.connect(gain).connect(ctx.destination);bass.start(now);bass.stop(now+.46);}
    window.setTimeout(() => void ctx.close(), count===4?1100:700);
  }, [musicOn]);
  useEffect(() => {
  ...
  return <main className={`app-shell ${reversed?'world-reversed':''}`}><div className="ambient" /><div className="character-snow" aria-hidden="true">{Array.from({length:15},(_,i)=><img key={i} src={`/characters/${['purple','blue','yellow'][i%3]}.png`} alt="" style={{'--x':`${(i*37)%96}%`,'--delay':`${-(i*1.7)%14}s`,'--duration':`${9+(i%6)*1.4}s`,'--size':`${42+(i%4)*15}px`} as React.CSSProperties}/>)}</div><header className="topbar"><div className="brand"><span className="brand-mark">R</span><h1>REVERSE<br/><b>TETRIS</b></h1></div><div className="header-actions"><button className="sound" aria-label={musicOn?'BGMをオフ':'BGMをオン'} onClick={()=>setMusicOn(v=>!v)}>{musicOn?<Volume2 size={17}/>:<VolumeX size={17}/>}</button><div className="top-score"><small>SCORE</small><strong>{score.toLocaleString()}</strong></div></div></header>
    {screen === 'start' ? <section className="start-card"><PixelTitle/><p className="eyebrow">FLIP THE GRAVITY</p><h2>積み上げた世界を<em>ひっくり返せ</em></h2><p className="intro">リバースブロックでラインを消すと盤面と重力が反転</p><label className="mode-toggle"><MinoBadge kind="l" color="#b36cff"/><span className="mode-copy"><b>リバースモード</b><small>反転コンボでスコア倍率アップ</small></span><input type="checkbox" checked={reverseEnabled} onChange={e=>setReverseEnabled(e.target.checked)} /><i /></label><button className="primary" onClick={start}><MinoBadge color="#ffdc39"/><span><Play fill="currentColor" size={19}/> ゲームスタート</span><MinoBadge kind="l" color="#ff557c"/></button><p className="hint">画面ボタンとキーボードの矢印キー スペースキーに対応</p></section> : <section className="game-layout">
